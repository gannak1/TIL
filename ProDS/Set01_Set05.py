...
q2.argmax() # 최대값이 있는 위치 번호 리턴
q2.idxmax() # 최대값이 있는 인덱스명

#=========================STU==================================================
round(data1.corr().Sales.TV,5)
#%%
...
from sklearn.linear_model import LinearRegression # 회귀계수만 리턴
from statsmodels.formula.api import ols # P-value를 알고싶을때 상세한 베이스 군산분석도가능
...
lm.fit(q3[var_list],q3.Sales) # 절편을 넣음
...
# form :'y~x1+C(x2)+x3-1' # -1을 넣으면 intercept 제거 # C(x2)로 범주형 데이터도 사용 가능
...
# OLS : 절편(intercept) 미포함
# -> 절편을 구할 수 있도록 1만 들어가 있는 상수항 변수를 추가
...
# - 검정은 이분산을 가정하고 수행한다. -> t검정을 해야함
# - 검정통계량의 추정치는 절대값을 취한 후 소수점 셋째 자리까지 반올림하여
# 기술하시오.
# - 신뢰수준 99%에서 양측 검정을 수행하고 결과는 귀무가설 기각의 경우 Y로, 그렇지
# 않을 경우 N으로 답하시오. (답안 예시) 1.234, Y
# 2표본 t검정 사용 모집단 분산 정보를 모르기 때문에
# =============================================================================
q1.columns
...
q2_out = ttest_ind(g_m,g_f,equal_var=False) # 양수면 뒤에 숫자가 더 많다

round(abs(q2_out.statistic),3)

# [참고] 등분산 검장
bartlett(g_m,g_f) # -> pvalue
# H0 : 등분산 귀무가설
# H1 : 등분산이 아니다(이분산) 대립가설
q2_out.pvalue < 0.01 # 기각
...
q3_pred_pr = logit.predict_proba(test[var_list]) # female 0 male 1
...
# - 두 국가의 육류별 소비량을 연도기준으로 비교하는 대응표본 t 검정을 수행하시오. 결측치를 제거하고 사용
...
# q3_out.iloc[q3_out.r2_score.idxmax(),2]
...
lm = LinearRegression().fit(temp[['TIME']],temp[['Value']])
r2_score = lm.score(temp[['TIME']],temp['Value'])
pred = lm.predict(temp[['TIME']]).reshape(-1,)
temp['Value']
# (MAPE = Σ ( | y - y ̂ | / y ) * 100/n ))
mape = (abs(temp['Value'] - pred[0]) / temp['Value']).sum() * 100 / len(temp)
mape
q3_out.append([i,r2_score,mape])
...
train, test = train_test_split(q3,train_size = 0.7,random_state = 123)
